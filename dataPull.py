...
def get_aqi():
    ...
    if 'aqi' in response['data']:
        ...
        daily_forecast = response['data']['forecast']['daily']
    else:
    ...
    data = {'city': city, 'state': state, 'lat': lat, 'long': long, 'aqi': aqi, 'iaqi': iaqi, 'daily_forecast': daily_forecast}
    bson_data = json_util.loads(json_util.dumps(data))
